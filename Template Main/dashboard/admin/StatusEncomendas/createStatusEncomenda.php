...
if (!empty($formData['AddStatus'])) {
    $criarStatusEncomenda = new StatusEncomendas();
    $criarStatusEncomenda->setFormData($formData);
    $value = $criarStatusEncomenda->create();
...
<head>
    ...
    <title>Cadastrar Status de Encomenda</title>
    <style>
        .header-gradient {
            ...
            <form method="POST" action="" class="row g-3">
                <div class="col-md-12">
                    <label for="status" class="form-label">
                        <i class="fas fa-tag me-1"></i> Nome do Status de Encomenda
                    </label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                        <input type="text" name="status" class="form-control"
                            placeholder="Digite o nome do status de encomenda" required
                            value="<?= isset($formData['status']) ? htmlspecialchars($formData['status']) : '' ?>">
                    </div>
                </div>
